...
# --- KLAVIATURA ---
def main_keyboard():
    # Siz so'ragan barcha buyruqlar tugma ko'rinishida
    keyboard = [
        ['📊 Portfel', '💰 Hisobot'],
        ['📅 Kunlik', '📅 Haftalik', '📅 Oylik'],
        ['🍰 Pie Grafik', '📁 Kategoriyalar'],
        ['🎯 Maqsadlar', '⚙️ Sozlamalar']
    ]
...
def parse_text(text, user_id):
    ...
    if any(word in text for word in ["kirim", "oldim", "oylik", "+", "daromad", "tushdi"]):
...
async def send_chart(update, df, title, filename, chart_type='line'):
    ...
    if chart_type == 'line':
        ...
        cat_sum.plot(kind='pie', autopct='%1.1f%%', startangle=140)
        plt.ylabel('')
...
async def haftalik(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    df = pd.read_sql_query("SELECT date, type, amount FROM transactions WHERE user_id=? AND date >= ?", conn, params=(user_id, limit))
    conn.close()
    await send_chart(update, df, "4 Haftalik Tahlil", "weekly.png")
...
async def oylik(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    df = pd.read_sql_query("SELECT date, type, amount FROM transactions WHERE user_id=? AND date >= ?", conn, params=(user_id, limit))
    conn.close()
    await send_chart(update, df, "1 Yillik Tarix", "monthly.png")
...
async def list_categories(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    res = "📁 **Kategoriyalaringiz:**\n\n" + ("\n".join([f"• {c}" for c in cats]) if cats else "Hali qo'shilmagan.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Tugmalar mantiqi
    if text == "📊 Portfel":
        # Avvalgi javobdagi portfel funksiyasi
        await update.message.reply_text("💼 Investitsiya Portfeli bo'limi (Tez orada...)")
    elif text == "💰 Hisobot":
        await hisobot(update, context)
    elif text == "📅 Kunlik":
        await kunlik(update, context)
    elif text == "📅 Haftalik":
        await haftalik(update, context)
    elif text == "📅 Oylik":
        await oylik(update, context)
    elif text == "🍰 Pie Grafik":
        await pie_chart(update, context)
    elif text == "📁 Kategoriyalar":
        await list_categories(update, context)
    else:
        # Xarajat/Kirim tahlili
        res = parse_text(text, user_id)
        if res:
            t, a, cat = res
...
if __name__ == "__main__":
    ...
    print("🚀 Bot barcha tugmalar bilan ishga tushdi...")
